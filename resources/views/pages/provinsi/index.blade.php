<x-admin-layout>
    @push('script')
        @if ($errors->any())
            <script>
                $('#createProvinsi').modal('show');
            </script>
        @endif
    @endpush
    <div class="row">
        <div class="col-6">
            <h4 class="text-dark">Kelola Data provinsi</h4>
        </div>
        <div class="col-6">
    ...
    <div class="row mt-4 mb-2">
        <div class="col-12">
            ...
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createProvinsi">
                <i class="fa-solid fa-plus"></i> Tambah Data
            </button>

            <!-- Modal -->
            <div class="modal fade" id="createProvinsi" tabindex="-1" aria-labelledby="createProvinsiLabel"
                aria-hidden="true">
                    ...
                    <div class="modal-content">
                        <form action="{{ route('provinsi-store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title" id="createProvinsiLabel">Tambah Data provinsi</h5>
    ...
    <div class="row bg-white px-3 py-4">
        <div class="col-12">
            <table class="table">
                ...
                <tbody>
                    @forelse ($provinsis as $key => $provinsi)
                        <tr>
                            ...
                            <td>
                                <img style="width: 50px" alt="logo"
                                    src="{{ asset('storage') . '/' . $provinsi->logo }}" alt="">
                            </td>
                            <td>{{ $provinsi->lat }} {{ $provinsi->lang }}</td>
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <a href="{{ route('provinsi-edit', [$provinsi->slug]) }}"
                                        class="btn btn-warning">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('provinsi-destroy', [$provinsi->slug]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
